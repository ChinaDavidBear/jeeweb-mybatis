...
<div style="width: 50%;
    text-align: center;
    margin: 20px auto;">
    <article id="post-48181" style="text-align: left;"
             class="post-48181 post type-post status-publish format-standard hentry category-service category-faq">
        <header class="entry-header">
            <h1 class="entry-title">${tbForumPost.title}</h1>
        </header>

        <div class="entry-content">
            <p style="margin-top: 15px;text-align: right;">发布时间：${tbForumPost.createDate}</p>
            <div style="margin-top: 30px;border-top: 5px solid #3CCD1C;">

                <c:forEach items="${list}" var="item" varStatus="status">
                    <div>
                        <div style="width: 100%;height:35px;border-bottom: 1px dashed #ccc;">
                            <div style="height: 35px;width:88px;line-height: 35px;float: left ">
                                    ${status.index+1}楼
                            </div>
                            <div style="height: 35px;line-height: 35px;text-align: left;float: left; ">
                                    ${item.createBy.id}
                            </div>
                        </div>
                        <div style="width: 100%;border-bottom:  1px solid #0d8ddb">
                            <div style="    margin: 10px;">
                                <p style="font-size: 28px; text-indent: 20px;">${item.content}</p>
                            </div>
                            <div style="height: 20px;line-height: 20px;text-align: right;    margin: 10px;">
                                回复时间：${item.createDate}
                            </div>
                        </div>
                    </div>
                </c:forEach>
    ...
    <div style="margin-top: 30px;">
        <form  id="loginform"  method="get" class="form" action="/front/contentadd">

            <div class="body-content">

                <div class="form-group">
                    <div class="input-icon right"><i class="fa fa-user"></i>
                        <input name="id" type="hidden" value="${tbForumPost.id}">
                        <input name="content" class="form-control" placeholder="填写回复内容" required="">
                    </div>
                </div>
                <div class="form-group" style="margin-top:10px;">
                    <c:if test="${not empty sessionScope.FRONT_USER}">
                        <div style="text-align: center">
                            <button type="submit" style="width: 100%;" class="btn btn-success">回复
